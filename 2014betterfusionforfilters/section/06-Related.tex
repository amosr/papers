...
In the loop fusion literature, the ILP approach is considered ``optimal'' because it can find the clustering that minimises a global cost metric. In our case the metric is defined by the objective function of~\S\ref{s:ObjectiveFunction}. Besides optimal algorithms, there are also heuristic approaches. For example, Gao, Olsen and Sarkar~\cite{gao1993collective} use the maxflow-mincut algorithm to try to maximise the number of fused edges in the LDG.  Kennedy~\cite{kennedy2001fastgreedy} describes another greedy approach which tries to maximize the reuse of intermediate arrays, and Song~\cite{song2004improving} tries to reduce memory references.

Darte~\cite{darte1999complexity} formalises the algorithmic complexity of various loop fusion problems and shows that globally minimising most useful cost metrics is NP-complete. Our ILP formulation itself is NP-hard, though in practice we have not yet found this to be a problem.
