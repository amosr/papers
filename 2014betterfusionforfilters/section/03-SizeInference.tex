...
$\tau_{\_}$  \>$::$\> $name \rightarrow \tau$ \\
\\
$\tau_o$    \> $|$ \> $o = @fold@~f~n$      \> $\in binds$ \> $=$ \> $k_n$ \\
            \> $|$ \> $o = @map@_n~f~ns$    \> $\in binds$ \> $=$ \> $k_o$ \\
            \> $|$ \> $o = @filter@~f~n$    \> $\in binds$ \> $=$ \> $k_n$ \\
            \> $|$ \> $o = @gather@~i~d$    \> $\in binds$ \> $=$ \> $k_i$ \\
            \> $|$ \> $o = @cross@~a~b$     \> $\in binds$ \> $=$ \> $k_a \times k_b$ \\
            \> $|$ \> $o = @external@~ins$  \> $\in binds$ \> $=$ \> $@external@$ \\
\end{tabbing}
