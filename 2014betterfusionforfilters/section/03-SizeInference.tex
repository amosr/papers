...
Even though in general $k_{gts}$ is not equal to $k_{xs}$, the three nodes @gts@, @sum1@ and @sum2@ can all be fused together.
...
For any set of bindings $bs$, each name $n$ will have at most one transducer.
\[
...
For any set of bindings $bs$, each pair of names $a$ and $b$ will have at most one pair of parents $parents(a,b)$.
\[
