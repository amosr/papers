...
% No,
We now define a function $trans$, to find the parent transducer a combinator application. Since each name is bound to at most one combinator, we abuse terminology here slightly and write ``combinator $n$'' when refering to the combinator occuring in the binding of the name $n$.
...
The size inference constraints reject programs with multiple transducers per node, so
for valid programs, each name will have at most one transducer, and each pair of names will have at most one pair of parents.
This simplifies the formulation, as it only needs to check one pair of parents.
