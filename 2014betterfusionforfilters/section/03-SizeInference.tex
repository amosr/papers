...
\TODO{Double check: is this really a sufficient property, or only necessary?}
\textbf{Definition: valid constraints}.
The constraints of a function $f$ are \emph{valid}, if and only if each name $n$ in its bindings will have at most one transducer:
\[
...
These two properties are used in the integer linear programming formulation, when constructing the program.
