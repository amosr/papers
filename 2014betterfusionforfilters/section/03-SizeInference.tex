...
When giving the size scheme of an operator graph we use $:_s$ to indicate that this is a size scheme rather than a type scheme.
...
\begin{alltt}
filterLeft 
 : \(\forall\,k\sb{1}.\,\exists\,k\sb{2}.\) Array \(k\sb{1}\) Float
              \(\to\) (Array \(k\sb{1}\) Float, Array \(k\sb{2}\) Float)
\end{alltt}
...
The rules for constraint generation are shown in Figure~\ref{f:ConstraintGeneration}. The judgment ~~$\SizeB{\Gamma_1}{zs}{b}{\Gamma_2}{C}$ reads: ``Under environment~$\Gamma_1$, array variable $zs$ binds the result of $b$, producing a new environment $\Gamma_2$ and size constraints $C$''. The other judgment forms are similar. The environment $\Gamma$ has the following grammar:
$$
\Gamma @::=@ ~~\cdot ~~|~~ \Gamma,~ \Gamma ~~|~~ zs : k ~~|~~ k ~~|~~ \exists k
$$

The $\cdot$ and $\Gamma,~ \Gamma$ are the empty environment and environment concatenation as usual. The element $zs : k$ records the size $k$ of some array variable $zs$. A plain $k$ indicates that $k$ can be unified with other size types when solving constraints, whereas $\exists k$ indicates a ``rigid'' size variable that cannot be unified with other sizes. We use the $\exists k$ syntax because this variable will also be existentially quantified if it appears in the size scheme of the overall function.
...
\begin{itemize}
\item    To solve the constraints, keep finding pairs of atomic equality constraints where the same program size variable appears on the left, and perform an appropriate substitution into the whole constraint set. 
