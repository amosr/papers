...
\TODO{More justification of why we want to use a heavyweight technology like ILP to do the clustering. Use normalizeInc or one like it to argue that plenty of real programs will admit multiple valid clusterings, so we really want to do this via constraints and an objective function. Compare with prior work on typed fusion that only tries to minimise the number of clusters.}
\begin{code}
...
\TODO{Why is this ``the simple version''? Why present a simple version before the real one?} \TODO{We need to show a complete constraint system at the start. Maybe use a simple objective function to start with, then refine it in a later section.}
...
If there is no edge between $i$ and $j$, the constraint simplifies to $-N \le \pi_j - \pi_i \le N$, which effectively means there is no constraint on the $\pi$ values. Note, however, that other edges may still constrain $i$ or $j$ by transitivity. \ben{If there is ``no constraint'' on the $\pi$ values then why bother adding the constraint at all?}
...
If two nodes have different sizes or rates, but are both sub-rates of another rate, they \emph{can} be fused together if they are both fused with their generators. \ben{What is a subrate? How do we determine the subrates of each rate?}
\TODO{Expand}.
