...
We can simplify the integer linear program, and make the job of the ILP solver easier, by ommitting these constraints.
...
$clusterable(v,vs,es)$  \\
    \>$=$       \>$\{v' | v' \in vs$                                \\
    \>$\wedge$  \>$\forall p \in path(v,v') \cup path(v',v).\ \fusionpreventing \not\in p\}$\\
\end{tabbing}
\end{figure}
