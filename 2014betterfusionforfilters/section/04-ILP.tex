...
% \TODO{Refer to example code that demonstrates these different opportunities.}
...
It may seem that we could generate far fewer constraints and rely on transitivity of clustering equality $x_{i,j}$.
This means that for each pair $i,j$ we would only generate an $x_{i,j}$ and constraints, if $i$ or $j$ is a @filter@, or there is an edge between $i$ and $j$.
This would simplify the integer linear program, but would also modify the objective function not to \emph{minimise} on these removed $x_{i,j}$.
