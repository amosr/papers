...
\begin{code}
 ...
 normalizeInc xs
  = let incs = map  (+1)    us      (A2) (B1)
    ...
    in  norm
\end{code}
% swapped A and B, so that the first option matches with the left graph, the second with the right graph in the diagram
...
The @normalizeInc@ example demonstrates why it is, in general, not sufficient, to simply minimize the number of loops in a program: we have to assign weights to different types of loops and take those into account when choosing a possible clustering. Ordered typed fusion \cite{kennedy1993typedfusion} addresses this problem in the context of fusing imperative parallel and sequential loops, but also only in the absence of filter-like operations. Our problem can be directly mapped to an integer linear program with some integral, boolean and real variables, an objective function to minimise, and a set of contraints the variables must conform to. While finding a variable assignment that satisfies the constraints and is the minimal objective function is NP-hard, we can build on exisiting work here and use the best ILP-solver available, which tend to be sufficient for realistic problem sizes.  For larger problems, we can find an approximate answer, within say $10\%$ of the optimal answer, which still results in a very efficient, if not necessarily optimal program.
