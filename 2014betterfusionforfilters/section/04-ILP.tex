...
\\
           \> $x_{ij}$    \> $\le$ \> $c_i$           \>       \>            \> (for all edges from i to j)      \\
\\
           \> $x_{ij}$    \> $=$   \> $1$             \>       \>            \> (fusion-preventing edges from i to j)      \\
\\
           \> $x_{ij}$    \> $=$   \> $1$             \>       \>            \> (if $\tau_i \not= \tau_j \wedge gen_i=\emptyset \wedge gen_j=\emptyset$)                 \\
           \> $x_{ij}$    \> $\le$ \> $x_{i'j}$       \>       \>            \> (if $\tau_i \not= \tau_j \wedge gen_i=\{i'\}$) \\
           \> $x_{ij}$    \> $\le$ \> $x_{ij'}$       \>       \>            \> (if $\tau_i \not= \tau_j \wedge gen_j=\{j'\}$) \\
\end{tabbing}
...
\item
Alternatively:
we can say, only generate subtype constraints if there is no \emph{fusion preventing path} between $i$ and $j$.
