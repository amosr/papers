...
% -------------------------------------
Figure~\ref{fig:Fusion:Def:StepPair} defines function \ti{tryStepPair} which decides which of two processes to advance.

Clauses (PreferJump1) and (PreferJump2) are heuristics which prioritize processes that can perform a @jump@. This helps collect @jump@ instructions together, which are then easier for post-fusion optimization to handle (\S\ref{s:Optimisation}). As clause (PreferJump2) calls $\ti{tryStep}$ with the instructions in swapped order, if \ti{tryStep} succeeds we use $\ti{swaplabels}$ (from Figure~\ref{fig:Fusion:Utils}) to re-swap the labels in the result.
...
% AR: Mention why the return type is Maybe, even though we don't have room for "otherwise Nothing" case.
