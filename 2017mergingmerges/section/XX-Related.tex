...
This work aims to address the problems with existing combinator-based array fusion systems.
As stated in the introduction, neither pull-based or push-based fusion alone is sufficient.
...
Short cut fusion is an attractive idea, as it allows fusion systems to be specified with a single rewrite rule.
...
Push-based short cut fusion systems \emph{do} exist \cite{gill1993short}, but support only the intersection of pull and push: that is, neither @zip@ nor @unzip@ \cite{svenningsson2002shortcut,lippmeier2013data}.
...
For effectful input streams, for example reading from the network, this duplicated work can change the overall meaning of the program.
...
The main drawback of data flow fusion is that it only supports a limited set of combinators, and adding more is non-trivial.
...
Here, pull streams have the type @Source@ and are represented as a source that is always available to be pulled from, while push streams have the type @Sink@ and are represented as a sink that can always accept input.
Both systems rely on stream bindings being used linearly to ensure correctness, including boundedness of buffers and ordering of effects.
