...
Each combinator is defined as a stream process and expressed in terms of a simple imperative program with a local heap. The process pulls data from an aribrary number if input streams and pushes data to at least one output stream. The definition of the @group@ combinator which removes consecutive elements from its input stream is as follows:

\begin{alltt}
 group 
   = \(\lambda\) (s1: Stream a) (s2: Stream a). 
     \(\exists\) (first: Bool)  (last: a) (value: a).
\end{alltt}\vspace{-1em}
\begin{code}
     process
     { ins:    { s1 }
     , outs:   { s2 }
