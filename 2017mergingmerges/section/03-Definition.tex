...
Fig.~\ref{fig:Process:Eval:Inject} gives the rules for injecting values into processes. The judgment form $\ProcInject{p}{v}{c}{p'}$ reads ``given process $p$, injecting value $v$ into channel $c$ yields a new process $p'$''. The @injects@ form is similar, but operates on sets of processes rather than a single one.
