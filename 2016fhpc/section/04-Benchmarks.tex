...
At Ambiata, we are currently using Icicle in production over medium-sized datasets (around three hundred gigabytes uncompressed) that fit on a single disk.
...
Incremental computation is even more important for the distributed case, as it can potentially avoid copying terabytes of data over the network.
...
\begin{axis}[
	...
	ybar interval=0.7,
  symbolic x coords={R, wc, CPU1, grep, wc -l, cat, Run 372, Run 1, end}
]
...
\TODO{update to talk about single CPU number.
Single CPU version 30 times faster than R, and in the same ballpark as @wc@ and @grep@.
Because the memory usage is bounded, we have a better idea of how many we can run on a single machine without running out of memory and stepping on each other's toes.
This means we can scale up to get quite close to the disk speed.
This is not true of the R code: assuming perfect scaling, we would need at least 150 threads.
Furthermore, the single-threaded R script is already using all the memory, and it is not obvious whether memory usage will decrease linearly as the input is divided and more threads are added.
Blah~\cite{mcsherry2015scalability}
}
