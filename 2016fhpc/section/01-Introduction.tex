...
\item
We present an fold-based intermediate language, which allows the query fusion transformation to be a simple matter of appending two intermediate programs\sref{s:IcicleCore};

\item
We present production benchmarks of Icicle compiled code, which compare favourably against standard unix utilities, and outperform an existing R feature generation system by several orders of magnitude\sref{s:Benchmarks}.
\end{itemize}
