...
In functional languages, fusion systems such as stream fusion\cite{coutts2007stream} and fold/build fusion\cite{jones2001playing}\TODO{correct?} rely on the inliner to move producers into their consumers, after which rewrite rules can remove intermediate allocations.
...
However, when dealing with data too large to fit in memory such as tables on disk, intermediate arrays becomes essential rather than just desirable.
...
Sadly, this program requires buffering the entire @xs@ source in memory while performing the first actions, then looping through the buffered @xs@ anf performing the second action afterwards.
...
It may not be immediately obvious just looking at the program that it requires two iterations.
