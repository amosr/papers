...
\TODO{Although, all the combinators so far use the whole input, and anyway checking for @Done@ of machine is too fine-grained if machine already has multiple combinators; perhaps an ``I promise not to read from X any more'' state would be interesting. Actually, @zip@ doesn't consume the whole input.}
