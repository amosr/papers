...
The trigger-fetch logic uses the \emph{count_when} streaming function to define the index; the increment flag given to \emph{count_when} will increment the index if the previous index has expired or is inactive in the current cycle.
