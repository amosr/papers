...
Blessing replaces the property statuses in an expression so that all checks and contracts are valid ($\PSValid$).
Applying blessing requires a proof that for all input streams, the checked semantics hold.
...
Causality checks that recursive streams are guarded by a followed-by delay.
