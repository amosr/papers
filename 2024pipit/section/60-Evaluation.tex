...
\TODO{EVALUATION NEEDS DOING:}
\begin{itemize}
  \item describe time-triggered CAN implementation
  \item whole implementation includes the extended version of trigger logic
  \item verified machine arithmetic
  \item for which we can generate real-time C code
  \item describe coffee machine controller
  \item simple, but fully verified according to an abstract spec
  \item and for which we generate real-time C code
\end{itemize}
...
Pipit generates real-time C code for this example\footnote{For a video of the controller in action, see \COFFEEURL}.
