...
\begin{itemize}
  ...
  \item we describe an encoding of transition systems that can express under-specified rely-guarantee contracts as functions, rather than relations, which results in simpler transition systems (\autoref{s:transition});
  \item we identify and mechanise the invariants and lemmas required to prove that the abstract transition system is an abstraction of the original semantics (\autoref{s:transition:proof});
  \item similarly, we verify that the executable transition system preserves the original semantics (\autoref{s:extraction});
